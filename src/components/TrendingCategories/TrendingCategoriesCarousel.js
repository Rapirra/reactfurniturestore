...
export default function TrendingCategoriesCarousel() {
    ...
    useEffect(() =>{
        const getItems = () => {
            fetch("data.json")
             ...
             .then(json => setData(json.trendingcarousel))
            }  
        getItems()
    },[])
  ...
  return (
    <div className='flex row w-[90%] mx-auto' style={{marginTop: "80px"}}>
       ...
       {data && data.map((item) => (
            <div className="card-top cursor-pointer trendingcarousel justify-center h-full px-[26px] py-[20px]" style={active===item.category ? {background: "#FBF9F3", border: "0.5px solid #FBF9F3 "} : null }  key={item.id} id={item.id} onClick={() => handleId(item.category)} >
